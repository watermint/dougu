...
impl Archive {
    ...
    // Convenience method to automatically determine provider based on file extension
    pub async fn create_archive_auto(
        ...
        let ext = archive_path.extension()
            .and_then(|e| e.to_str())
            .ok_or_else(|| anyhow!(error_messages::INVALID_ARCHIVE_FORMAT))?;
            
        let provider = self.get_provider_for_extension(ext)
            .ok_or_else(|| anyhow!(error_messages::PROVIDER_NOT_FOUND))?;
...
mod tests {
    use super::*;
    use crate::providers::zip::ZipProvider;
    ...
    fn it_registers_provider() {
        ...
        assert_eq!(archive.providers.len(), 1);
        assert!(archive.get_provider("zip").is_some());
    }
} 